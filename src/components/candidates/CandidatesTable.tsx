...
function VerdictPanel({
  ...
  const vClass = verdict.v === 'ENTER' ? 'vp-verdict vp-verdict-enter'
               : verdict.v === 'WAIT'  ? 'vp-verdict vp-verdict-wait'
               :                           'vp-verdict vp-verdict-skip'

  function sigColor(color: string) {
    if (color.includes('gain')) return 'var(--green)'
    if (color.includes('warn')) return 'var(--amber)'
    if (color.includes('loss')) return 'var(--red)'
    return 'var(--text2)'
  }
  ...
  return (
    <div className="vp-shell">
      ...
      <div className="vp-levels">
        <div className="vp-level-tile">
          <div className="vp-tile-label">Entry zone (indicative)</div>
          <div className="vp-tile-value" style={{ color: 'var(--blue)' }}>
            ${levels.entryLow.toFixed(2)} – ${levels.entryHigh.toFixed(2)}
          </div>
          <div className="vp-tile-sub">1H EMA 20/50 retest — confirm on chart</div>
        </div>
        <div className="vp-level-tile">
          <div className="vp-tile-label">Stop · T1 · T2 (indicative)</div>
          <div className="vp-tile-value" style={{ color: 'var(--red)' }}>${levels.stop.toFixed(2)}</div>
          <div className="vp-tile-sub" style={{ color: 'var(--green)' }}>
            T1 ${levels.t1.toFixed(2)} · T2 ${levels.t2.toFixed(2)}
          </div>
        </div>
        <div className="vp-level-tile">
          <div className="vp-tile-label">Size · Risk · R:R</div>
          <div className="vp-tile-value">€{Math.round(levels.posEur)} · {levels.shares} shares</div>
          <div className="vp-tile-sub" style={{ color: levels.rr < 2 ? 'var(--red)' : 'var(--amber)' }}>
            Risk €12 · R:R {levels.rr.toFixed(1)}:1{levels.rr < 2 ? ' ⚠ below minimum' : ''}
          </div>
        </div>
      </div>

      {/* ── Signals — 2 columns ── */}
      <div className="vp-signals">
        {signals.map(s => (
          <div key={s.key} className="vp-signal-card">
            <div className="vp-signal-key">{s.key}</div>
            <div className="vp-signal-val" style={{ color: sigColor(s.color) }}>{s.val}</div>
            <div className="vp-signal-note">{s.note}</div>
          </div>
        ))}
      </div>
      ...
      <div className={vClass}>
        <div className="vp-verdict-label">{verdict.label}</div>
        <div className="vp-verdict-text">{verdict.text}</div>
      </div>

      {/* ── State actions ── */}
      <div className="vp-actions">
        <span className="vp-action-label">Mark as:</span>
        {(['SETUP_CONFIRMED', 'INVALIDATED', 'TRADED', 'CANDIDATE'] as const).map(state => (
          <button
            key={state}
            disabled={isPending}
            onClick={e => { e.stopPropagation(); onStateChange(state) }}
            className={`vp-state-btn${c.candidateState === state ? ' active' : ''}`}
          >
            {state.replace('_', ' ')}
          </button>
        ))}
